...
const cadState={projects:[],activeProject:null,triangles:[],bounds:null,rx:-.58,ry:.72,zoom:1,dragging:false,last:[0,0],showSolid:true,showWire:true,showDims:true,raf:null};
...
function normalizeTris(tris){const bounds=computeBounds(tris);const scale=2.45/Math.max(...bounds.size,.01);return{tris:tris.map(tr=>tr.map(p=>p.map((v,i)=>(v-bounds.center[i])*scale))),bounds};}
...
async function loadStlProject(project){cadState.activeProject=project;updateCadInfo(project);if(!project.files?.stl){cadState.triangles=normalizeTris(fallbackTris()).tris;cadState.bounds=null;const d=document.getElementById('cadDimensions');if(d)d.textContent='CTB only';statusText('This entry is a CTB/manufacturing file. Viewer shows reference cube until an STL is added.','warn');return;}statusText('Loading STL mesh…');try{const buffer=await fetch(project.files.stl).then(r=>{if(!r.ok)throw new Error(r.status);return r.arrayBuffer()});const parsed=decodeSTL(buffer);if(!parsed.length)throw new Error('No triangles found');const norm=normalizeTris(parsed);cadState.triangles=norm.tris;cadState.bounds=norm.bounds;updateCadDimensions(norm.bounds);statusText('STL loaded. Drag to rotate, scroll to zoom.');}catch(e){cadState.triangles=normalizeTris(fallbackTris()).tris;cadState.bounds=null;updateCadDimensions(null);statusText('STL file not found yet. Place assets in /cad/ paths or run with a local server.','warn');}}
...
function drawCadViewer(){const canvas=document.getElementById('stlCanvas');if(!canvas)return;const ctx=canvas.getContext('2d');const ratio=Math.min(devicePixelRatio||1,2);const rect=canvas.getBoundingClientRect();canvas.width=Math.max(320,rect.width)*ratio;canvas.height=Math.max(260,rect.height)*ratio;ctx.setTransform(ratio,0,0,ratio,0,0);const w=canvas.clientWidth,h=canvas.clientHeight;ctx.clearRect(0,0,w,h);const accent=getComputedStyle(document.documentElement).getPropertyValue('--glow-rgb').trim()||'26,167,255';ctx.globalAlpha=.42;ctx.strokeStyle=`rgba(${accent},.18)`;ctx.lineWidth=1;for(let x=36;x<w;x+=36){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke()}for(let y=36;y<h;y+=36){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke()}ctx.globalAlpha=1;const project=p=>{let[x,y,z]=p;const cy=Math.cos(cadState.ry),sy=Math.sin(cadState.ry),cx=Math.cos(cadState.rx),sx=Math.sin(cadState.rx);const x1=x*cy-z*sy,z1=x*sy+z*cy,y1=y*cx-z1*sx,z2=y*sx+z1*cx+4.2,s=Math.min(w,h)*.35*cadState.zoom/z2;return[w/2+x1*s,h/2-y1*s,z2]};const tris=cadState.triangles.length?cadState.triangles:normalizeTris(fallbackTris()).tris;const faces=tris.map(tr=>{const pts=tr.map(project);return{pts,z:pts.reduce((a,p)=>a+p[2],0)/3}}).sort((a,b)=>b.z-a.z);faces.forEach((f,idx)=>{ctx.beginPath();ctx.moveTo(f.pts[0][0],f.pts[0][1]);ctx.lineTo(f.pts[1][0],f.pts[1][1]);ctx.lineTo(f.pts[2][0],f.pts[2][1]);ctx.closePath();if(cadState.showSolid){const shade=Math.max(.10,Math.min(.32,.28-(idx/faces.length)*.14));ctx.fillStyle=`rgba(${accent},${shade})`;ctx.fill()}if(cadState.showWire){ctx.strokeStyle='rgba(220,245,255,.72)';ctx.lineWidth=.85;ctx.stroke()}});if(cadState.showDims&&cadState.bounds){ctx.save();ctx.fillStyle='rgba(0,0,0,.42)';ctx.strokeStyle=`rgba(${accent},.58)`;ctx.lineWidth=1;const text=`X ${fmtMm(cadState.bounds.size[0])}   Y ${fmtMm(cadState.bounds.size[1])}   Z ${fmtMm(cadState.bounds.size[2])}`;ctx.font='700 13px Inter, system-ui, sans-serif';const tw=ctx.measureText(text).width+28,x=18,y=h-48;ctx.beginPath();ctx.roundRect(x,y,tw,32,12);ctx.fill();ctx.stroke();ctx.fillStyle='#fff';ctx.fillText(text,x+14,y+21);ctx.restore()}cadState.raf=requestAnimationFrame(drawCadViewer);}
